...
def Kcat_predict(Ifeature, Label, sequence_new, Smiles_new, ECNumber_new, Substrate_new, Type_new, Temp_k_norm_new, Inv_Temp_norm_new, Temp_new, Temp_k_new, Inv_Temp_new):
    ...
    for train_index, test_index in kf.split(Ifeature):
        ...
        # dvalid = xgb.DMatrix(Ifeature)
        # model = xgb.train(param, dtrain, int(num_round), verbose_eval=False)
        # Pre_all_label = model.predict(dvalid)
        ...
        res.to_csv(f'PreKcat_new/K_Fold/PreTKcat/{fold}_PreTKcat_DLTKcat_Sum_all_samples_metrics.csv', index=False)
        # res.to_csv(f'PreKcat_new/K_Fold/PreTKcat/{fold}_PreTKcat_DLTKcat_Sum_NoT_all_samples_metrics.csv', index=False)
